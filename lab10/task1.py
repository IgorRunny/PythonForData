...
for index, column in enumerate(columns):
    header_cells[index + 1].text = column
    header_cells[index + 1].paragraphs[0].runs[0].font.bold = True
    header_cells[index + 1].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER

for index, record in enumerate(table_data):
    row_cells = table.add_row().cells
    row_cells[0].text = rows[index]
